...
def CLAHE(image, clipLimit=2.0, tileGridSize=(8,8)):
  """
  CLAHE image enhancement by histogram
  Args:
      image: numpy ndarray of input image
      clipLimit: the scalar number of clipLimit for creating CLAHE object
      tileGridSize: tuple grid size for creating CLAHE object
  Returns:
      img_clahe: ndarray CLAHE computed input image
  """
...
def GHE(image):
  """
  GHE image enhancement by histogram
  Args:
      image: numpy ndarray of input image
  Returns:
      img_ghe: ndarray GHE computed input image
  """
  img_gray = image.convert('L')
  img_arr = np.array(img_gray)
...
def BBHE(image):
  """
  BBHE image enhancement by histogram
  Args:
      image: numpy ndarray of input image
  Returns:
      bbhe_img: ndarray BBHE computed input image
      hist_bbhe: ndarray computed input image histogram
  """
  img_gray = image.convert('L')
  img_arr = np.array(img_gray)
